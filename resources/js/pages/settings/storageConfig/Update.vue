            ...
            <div class="flex flex-col space-y-6">
                ...
                <form @submit.prevent="form.post(route('settings.storage.update', {config: storageConfig.data}))" class="flex flex-col space-y-6">
                    ...
                    <div>
                        ...
                        <select id="provider_type" name="provider_type" v-model="form.provider_type" @change="form.provider_options = {}">
                        ...
                        <div v-if="form.errors.provider_type">{{ form.errors.provider_type }}</div>
                    </div>

                    <div class="flex flex-col space-y-4">
                        <CreateWebDavForm v-if="form.provider_type === 'webdav'"
                                          :form="form as unknown as InertiaForm<CreateStorageConfigForm<CreateWebDavFormData>>" />
                        <CreateDropboxForm v-if="form.provider_type === 'dropbox'"
                                           :form="form as unknown as InertiaForm<CreateStorageConfigForm<CreateDropboxFormData>>"/>
                    </div>
