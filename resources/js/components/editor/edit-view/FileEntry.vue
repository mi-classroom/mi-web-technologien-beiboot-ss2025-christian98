            ...
            <DropdownMenu>
                ...
                <DropdownMenuPortal>
                    <DropdownMenuContent align="end">
                        <DropdownMenuItem title="Save attribute for file" @click="$emit('save', item ?? file, newValue)">
                        ...
                        <DropdownMenuItem v-if="item" title="Remove attribute from file" @click="$emit('remove', item)">
    ...
    <div class="mt-2 flex flex-col gap-1">
        <div v-for="(_, idx) in newValue" class="flex items-end gap-1 **:data-heading:first:not-sr-only">
            <div class="flex grow gap-1">
                <div class="basis-1/2">
                    <label :for="`iptc-${selectedTag}-${idx}-current`" class="px-0.5 text-xs/5 text-gray-500 sr-only"
                           :data-heading="idx === 0 ? true : undefined">
                        Current value
                    </label>
                    <input :id="`iptc-${selectedTag}-${idx}-current`" :name="`iptc-${selectedTag}-${idx}-current`" type="text"
                ...
                <div class="basis-1/2">
                    ...
                    <input :id="`iptc-${selectedTag}-${idx}-new`" :name="`iptc-${selectedTag}-${idx}-new`" type="text"
