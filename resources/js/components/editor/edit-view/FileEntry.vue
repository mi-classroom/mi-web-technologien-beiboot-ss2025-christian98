...
const item = computed(() => {
    return props.file.meta_data?.iptc_items?.find(
        (i) => i.tag.id === props.selectedTag.id,
    );
});
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuPortal>
                        <DropdownMenuContent align="end">
                            <DropdownMenuItem
                                title="Save attribute for file"
                                @click="save">
                                <Icon name="Save" class="size-4" />
                                <span>Save file</span>
                            </DropdownMenuItem>
                            <DropdownMenuItem
                                title="Reset attribute for file"
                                @click="
                            ...
                            <DropdownMenuItem
                                v-if="item"
                                title="Remove attribute from file"
        ...
        <div class="mt-2 flex flex-col gap-1">
            <div
                ...
                <div class="flex grow gap-1">
                    <div class="basis-1/2">
                        <label
                            :for="`iptc-${selectedTag.id}-${idx}-current`"
                            class="sr-only px-0.5 text-xs/5 text-gray-500"
                            :data-heading="idx === 0 ? true : undefined">
                            Current value
                        </label>
                        <IptcInputFactory
                            :model-value="item?.value[idx]"
                            :definition="selectedTag"
                            :index="idx"
                            title="Current Value"
                            suffix="current"
                            readonly />
                    ...
                    <div class="basis-1/2">
                        <label
                            :for="`iptc-${selectedTag.id}-${idx}-new`"
                            class="sr-only px-0.5 text-xs/5 text-gray-500"
                            :data-heading="idx === 0 ? true : undefined">
                            New value
                        </label>
